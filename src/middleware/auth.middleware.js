...
export const isAuthenticated = ({tokenType = tokenTypes.access} = {}) => {
    ...
    const payload = verifyToken({ token: token , signature: tokenType === tokenTypes.access ? process.env.ACCESS_TOKEN_SIGNATURE : process.env.REFRESH_TOKEN_SIGNATURE }) // verify throw error automatically
    // check token in DB
